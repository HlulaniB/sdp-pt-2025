...
function AuthProvider({ children }) {
    ...
    const createUserInNeon = async (firebaseUser, additionalData = {}) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.error || "Failed to create user in database",
                );
            }
    ...
    const migrateExistingUser = async (firebaseUser) => {
        try {
            console.log(
                `Checking if user ${firebaseUser.email} exists in Neon...`,
            );

            const response = await fetch(
                `${API_BASE_URL}/users/${firebaseUser.uid}/migrate`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (firebaseUser) {
                try {
                    ...
                    if (!userData) {
                        ...
                        await fetch(
                            `${API_BASE_URL}/users/${firebaseUser.uid}`,
                            {
                                method: "PATCH",
                                headers: {
                                    "Content-Type": "application/json",
                    ...
                    const isProfileComplete =
                        checkUserProfileComplete(userData);
                    setUser({
                        ...firebaseUser,
